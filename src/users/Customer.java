...
/*    Application.rs=Application.stmt.executeQuery("select employee.emp_name,servicereln.license_no,servicereln.service_id,requires.quantity,repair.actual_fees,repair.diagnostic_fees,repair.repair_id,timeslot.mechanic_id,timeslot.service_date,timeslot.service_time,timeslot.end_date,timeslot.end_time,requires.part_id,parts.price,parts.warranty from parts,employee,repair, requires,servicereln,timeslot,mechanic where servicereln.customer_id='"+id+"' AND timeslot.service_id=servicereln.service_id AND servicereln.service_id = repair.service_id AND timeslot.mechanic_id= mechanic.mechanic_id AND mechanic.emp_id=employee.emp_id AND servicereln.service_id=requires.service_id AND requires.part_id=parts.part_id");
    ...
    private void findRescheduleDate(int serviceID) throws Exception {
        List<Date> dates = new ArrayList<>();
        System.out.println("Choose whether reschedule is for maintenance or for repair");
        ...
        case 1: Application.rs=Application.stmt.executeQuery("select license_no from servicereln where service_id="+serviceID);
                ...
                scheduleMaintenanceProcess(plate,mileage,mname);break;
    ...
    // This method is called for scheduling maintenance
	...
	private void scheduleRepairProcess(int problem, String licensePlate, int currentMileage, String mechanicName) throws Exception {
...
/*    // query db & check if an order exists that satisfies parts requirements
    ...
    private void scheduleRepair(String licensePlate, int currentMileage, String mechanicName) throws Exception {
        ...
        if(option >= 1 && option <= 7) {
			scheduleRepairProcess(option, licensePlate, currentMileage, mechanicName);
 //else {
...
/*                if(!checkPartsSufficient()) {
    ...
    private void createRepairService(int problem, Map<Date, Map<String[], Integer>> datesResult, Date chosenDate, String licensePlate, float actualFees, float laborCharge, float totalTime, Map<String, Map<Integer, Integer>> scParts) throws Exception {
		...
		// Insert : APPOINTMENT
		String insertAppointment = "insert into APPOINTMENT(APPOINTMENT_ID, SERVICE_TYPE, CUSTOMER_ID, SC_ID) values(" + appID + ", '', " + this.userID + ", '" + findCustomerServiceCenter() + "')";
		Application.stmt.executeUpdate(insertAppointment);
